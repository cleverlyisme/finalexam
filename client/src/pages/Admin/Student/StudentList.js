...
const StudentList = (props) => {
  ...
  const getData = async () => {
    try {
      ...
      toast.error("Lỗi trong khi tải danh sách học sinh");
    }
  };
  ...
  useEffect(() => {
    if (role === "teacher") {
      let data = null;
      if (
        user?.teacherOfClasses.find(
          (item) => item._id === user?.mainTeacherOfClass._id
        )
      ) {
        data = user?.teacherOfClasses.map((item) => item.name);
      } else {
        data = [
          ...user?.teacherOfClasses.map((item) => item.name),
          user?.mainTeacherOfClass.name,
        ];
      }
  ...
  const renderModalNote = () => {
    return (
      <ViewModal
        ...
        title={`Cập nhật ghi chú học sinh ${student.fullName}`}
